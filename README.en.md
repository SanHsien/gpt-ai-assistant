...
- **`6.0.0-rc.5`** retains the durable-only, toolchain, container, and command-entry baseline while adding an explicit Google Tasks API deployment prerequisite and safe revival of a dead Tasks sync job after a permanent setup error is fixed. Final `6.0.0` still requires the consolidated LINE/Google acceptance pass.
