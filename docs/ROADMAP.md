...
- [x] 語音沿用現有 transcription，送入與文字**完全相同**的 event-draft 流程：LINE 音訊訊息經 `transcribeAudio`（Whisper／`gpt-4o-mini-transcribe`）轉文字後即成 `trimmedText`，`app/app.js` 已收 `isAudio`、schedule handler 無 `isText` 閘門，指令（`記行程 …`）與隱式日期語句皆可觸發，走同一 parse→durable 追問→確認→CRUD→Google 同步→提醒。rc.9 也接收 LINE 桌面版 `file` webhook 的音訊白名單與大小限制。語音確認卡額外回顯「🎤 我聽到：…」原文，讓使用者分辨聽錯與解析錯（`__TEXT_SCHEDULE_HEARD`）。
...
`5.13.0` 先完成不破壞相容性的收斂；`6.0.0-rc.9` 已落地 breaking runtime 契約、feature-aware LINE 快捷入口、Node 24 容器可靠性、Express／Jest／ESLint 維護基線、Google Tasks 永久設定錯誤恢復、週期行程當地鐘點校正、Google request／cron drain time budget、Calendar inbound 非展開系列同步，以及 LINE 桌面音訊檔入口；既有 Production 升級與回滾往返已通過，現進入集中驗收：
