...
**下一步**：`6.0.0-rc.6` 完成 durable-only 程式收斂、LINE 快捷入口、Google OAuth locale、Node 24 container healthcheck、Express 5／Jest 30／ESLint 10 維護基線、Google Tasks API 前置與 dead sync job 安全恢復，以及週期行程當地鐘點校正；Production `0018`、RC health、每分鐘 Cron 與 5.x ↔ RC 回滾往返均已通過。跑完 [`REVIEW.md`](../REVIEW.md) 的剩餘真實 LINE／Google 驗收後，才將同一候選升為正式 `6.0.0`。
