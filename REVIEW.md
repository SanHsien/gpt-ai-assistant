...
6.0 的程式收斂、維護基線與集中實機驗收已完成；`6.0.1` 補上正式驗收清理時發現的行程刪除與提醒一致性修正。
...
所有 Release Gate 均已通過；`6.0.1` 為不改驗收步驟的資料一致性 patch。
