...
6.0 的程式收斂、維護基線與集中實機驗收已完成；`6.0.1` 補上正式驗收清理時發現的行程刪除與提醒一致性修正。`6.1.0` 讓 primary Google Calendar 既存的未來單次 timed 行程與有期限任務共用 LINE 提醒，並完成 Production migration、Cron、Google inbound、實際 LINE 投遞與清理閉環。2026-08-08 另以 LINE Windows app 重跑 Google OAuth 與行程同步 smoke test，LINE、Calendar、Supabase、Vercel 及精準清理再次完成閉環；同日收斂所有 Dependabot PR 與 freshness issue，`google-auth-library` 升上 major `11` 並由該次真實 OAuth 授權在 Production 驗證。
...
- `npm test -- --runInBand`：80 suites、588 tests 全部通過；新增依賴 freshness、核准暫緩版本邊界、Dependabot 風險政策、guarded merge workflow 與 transaction client 逐筆 await 回歸，既有週期行程、Google transport／worker time budget、Calendar sync query v2／v3、桌面音訊、durable、Quick Reply、完整 `指令` 與 Google contract 測試全數保留。
...
- **［已完成］依賴 PR／issue 收斂與 `google-auth-library` major 驗證（Claude，2026-08-08）**：合併 3 個 Dependabot PR——#10（`axios` `^1.2.1`→`^1.19.0`、**`google-auth-library` `^10`→`^11` major**）、#12（`undici` 6.28.0，涵蓋 GHSA-m8rv-5g2x-5cg5／GHSA-8xcm-r25x-g524／GHSA-v3r7-h72x-cjcm）、#11（`ip-address` 10.4.0）。**測試以 `jest.doMock` 完全 mock `google-auth-library`，故綠燈不覆蓋此 major**；另行實證：v11 具備使用中的 7 個 `OAuth2Client` API（`generateCodeVerifierAsync`／`generateAuthUrl`／`getToken`／`setCredentials`／`getAccessToken`／`request`／`revokeCredentials`）、`engines >=22` 相容本專案 `>=24`，並以原生 Node ESM 實際 `import` `services/google-calendar.js` 與 `google-tasks.js` 確認 named export 可解析（即先前 `rrule` CommonJS 那類 ESM 失敗模式不重現）。#11 的 Dependabot `prepare` 腳本警告經檢視為 `git config core.hooksPath hooks || true`，且 npm 從 registry 安裝依賴時不執行 `prepare`，無安裝期執行風險；套件由 GitHub Actions OIDC trusted publishing 發布。合併採 strict up-to-date 逐一 `update-branch`＋必要 checks 全綠後 squash。**2026-08-08 維護者以真實 LINE 完成 Google OAuth 授權閉環實測通過，v11 於 Production 正式驗證。**
- **［已完成］freshness tracker 歸零（Claude，2026-08-08，`a4dc604`）**：`@vercel/blob` 2.7.0、`@vercel/functions` 3.9.0、`express-rate-limit` 8.6.2 範圍內更新；`npm audit` 由 2 high 歸零（`js-yaml` 3.15.0→3.15.1 CVE-2026-59870、`brace-expansion` 5.0.8→5.0.9，皆 dev-only 間接依賴，比對合併前後版本確認非本批引入而是期間新公告）；評估後升 pinned devDependencies `eslint` 10.8.1、`globals` 17.9.0。Babel 7.x 維持不動（Jest 30 transform 鏈契約，屬已核准暫緩且未逾邊界）。推送後 `dependency-freshness` workflow 依其既定條件自動關閉 issue #5（`COMPLETED`），main 上 `test`／`docker-smoke`／CodeQL 全綠。文件站同批合併 `undici` 7.29.0、`js-yaml` 3.15.1、`postcss` 8.5.26（皆 dev-only），本機 `npm audit` 0 且 build 通過 13 頁／362 連結。兩個 repo 現皆只餘 `main`、0 open PR。
