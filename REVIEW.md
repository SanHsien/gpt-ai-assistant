...
最後覆核：2026-07-22，目標版本 `6.0.0-rc.11`。
...
- `npm test -- --runInBand`：74 suites、524 tests 全部通過；新增週期行程明確鐘點、次日起算、直接路由、可見確認、Google transport timeout、worker time budget、Calendar sync query v2，以及桌面音訊檔路由／大小限制／實際格式判斷／語音同音指令回歸，既有 durable／Quick Reply／完整 `指令`／Google contract 測試全數保留。
...
- [ ] rc.11 由 LINE PC 附加支援音訊檔，確認轉錄回顯、行程草稿、Google 單筆建立與清理。rc.9 已證明 LINE Windows 會把桌面 WAV／MP3 轉成 `audio` webhook；rc.10 修正格式後仍因「記」被辨成「寄」而落入一般聊天。語音句首同音字容錯已於 2026-07-22 以 `2cbef0d` 修復並保留原始回顯，待部署後重驗。

以上通過後，才把 package／release／docs 由 `6.0.0-rc.11` 提升為正式 `6.0.0`。
