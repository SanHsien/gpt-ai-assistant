...
最後覆核：2026-07-18，目標版本 `6.0.0-rc.7`。
...
- `npm test -- --runInBand`：72 suites、512 tests 全部通過；新增週期行程明確鐘點、次日起算、直接路由、可見確認、Google transport timeout 與 worker time budget 回歸，既有 durable／Quick Reply／完整 `指令`／Google contract 測試全數保留。
...
- [ ] rc.7 部署後連續觀察 Cron；rc.6 實機期間 Google Calendar inbound 曾兩次拖滿 Vercel 60 秒，rc.7 已加入 10 秒 provider timeout 與 45 秒 drain budget。
- [ ] 真實 LINE 語音訊息轉錄、草稿確認與 Google 單筆建立。桌面自動化不能等同 LINE `audio` webhook，需最後由使用者送一則語音。

以上通過後，才把 package／release／docs 由 `6.0.0-rc.7` 提升為正式 `6.0.0`。
