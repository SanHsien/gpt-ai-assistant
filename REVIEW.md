...
最後覆核：2026-07-18，目標版本 `6.0.0-rc.6`。
...
- `npm test -- --runInBand`：72 suites、510 tests 全部通過；新增週期行程明確鐘點、次日起算、直接路由與可見確認規則回歸，既有 durable／Quick Reply／完整 `指令`／Google contract 測試全數保留。
...
- [ ] rc.6 部署後重驗週期行程：當地鐘點正確、確認卡顯示規則、Google 只建立一個 series、下一 occurrence reminder 不重複。
- [ ] `REMINDER_OFFSETS=60,1440` 的提前 60 分鐘提醒；23:38 測試行程於 22:38 未收到提前提醒，先校正 Production 設定後重驗。
- [ ] 真實 LINE 語音訊息轉錄、草稿確認與 Google 單筆建立。桌面自動化不能等同 LINE `audio` webhook，需最後由使用者送一則語音。

以上通過後，才把 package／release／docs 由 `6.0.0-rc.6` 提升為正式 `6.0.0`。
