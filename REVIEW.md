...
- RC.4 Vercel Production 為 `READY`、Node `24.x`；穩定網域與 `/health/live` 實測均回 `200`，`currentVersion`／`latestVersion` 都是 `6.0.0-rc.4`。先前 `5.13.0` ↔ RC promote／rollback 往返仍是已通過基線。
